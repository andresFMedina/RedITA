...
class VideoPlayerRecyclerView : RecyclerView {
    ...
    fun playVideo(isEndOfList: Boolean) {
        ...
        val dataSourceFactory: DataSource.Factory = DefaultDataSourceFactory(
            ctx, Util.getUserAgent(ctx, "RecyclerView VideoPlayer")
        )
        val mediaUrl: String = mediaObjects[targetPosition].mediaUrl
    ...
    private fun animateVolumeControl() {
        if (volumeControl != null) {
            ...
            if (volumeState == VolumeState.OFF) {
                requestManager?.load(R.drawable.ic_volume_off_gray_24dp)
                    ?.into(volumeControl!!)
            } else if (volumeState == VolumeState.ON) {
                requestManager?.load(R.drawable.ic_volume_up_gray_24dp)
                    ?.into(volumeControl!!)
            }
